...
.DESCRIPTION
    ...
    Affiche l'état actuel, propose la correction semaine 06:01 -> 16:00, demande confirmation.
.PARAMETER Server
    Instance SQL Server (obligatoire en production).
.PARAMETER Database
    Nom de la base (défaut : fpusnr).
.PARAMETER Username
    Utilisateur SQL (obligatoire si non fourni via invite).
.PARAMETER Password
    Mot de passe SQL (obligatoire si non fourni via invite ; ne pas versionner).
.EXAMPLE
    cd livraison\2026-05-19
    .\Update-ShiftTimes.ps1 -Server VM-SQL -Database fpusnr -Username sa
#>
...
$queryUpdate = Read-SqlFile "update_shifttimes_weekdays_0601_1600.sql"
...
try {
    ...
    if ($reader.Read()) {
        ...
        $title = "Confirmation"
        $message = "Appliquer ces changements sur $Server ? (sql\update_shifttimes_weekdays_0601_1600.sql)"
